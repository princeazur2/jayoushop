        ...
        <div className="relative min-h-screen overflow-hidden bg-background pb-24">
            ...
            <section className="relative z-10 mx-auto max-w-7xl px-4 py-8 md:px-8">
                <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 mb-8 md:mb-12">
                    <div className="no-scrollbar flex w-full flex-nowrap items-center gap-3 overflow-x-auto md:w-auto md:flex-wrap">
                        <button
                    ...
                    <div className="flex items-center gap-3 shrink-0 glass p-1.5 rounded-2xl self-end md:self-auto">
                        <div className="pl-3 text-primary">
                            <SlidersHorizontal className="h-4 w-4" />
                        </div>
                        <Select value={sort} onValueChange={(value) => setSort(value as typeof sort)}>
                            <SelectTrigger className="w-[160px] border-none bg-transparent shadow-none font-medium sm:w-[180px]">
                ...
                {loading ? (
                    <div className="grid grid-cols-2 gap-3 sm:gap-6 md:gap-8 lg:grid-cols-4">
                        {Array.from({ length: 4 }).map((_, i) => (
                            <div key={i} className="animate-pulse">
                                <div className="aspect-[4/5] rounded-2xl bg-muted md:rounded-3xl" />
                                <div className="mt-3 h-3 w-3/4 rounded bg-muted md:mt-4 md:h-4" />
                                <div className="mt-2 h-3 w-1/2 rounded bg-muted md:h-4" />
                            </div>
                        ))}
                    </div>
                    ...
                    <motion.div layout className="grid grid-cols-2 gap-3 sm:gap-6 md:gap-8 lg:grid-cols-4">
                                ...
                                return (
                                    ...
                                    >
                                        <Link
                                            ...
                                            className="group block overflow-hidden rounded-2xl glass shadow-sm transition-all duration-500 hover:-translate-y-2 hover:glow-primary md:rounded-3xl"
                                        >
                                            <div className="relative aspect-[4/5] overflow-hidden bg-muted/30">
                                                {!inStock && (
                                                    <span className="absolute left-2 top-2 z-10 rounded-full bg-foreground/90 px-2.5 py-1 text-[9px] font-bold uppercase tracking-wider text-white shadow-md md:left-3 md:top-3 md:px-3 md:text-[10px]">
                                                        Épuisé
                                                    </span>
                                                )}
                                                <img
                                            ...
                                            <div className="flex flex-col gap-0.5 p-3 md:gap-1 md:p-5">
                                                <span className="text-[9px] font-bold uppercase tracking-[0.15em] text-primary/70 md:text-[10px] md:tracking-[0.2em]">
                                                    {product.categories?.name}
                                                </span>
                                                <h3 className="font-display text-sm leading-snug text-foreground group-hover:text-primary transition-colors line-clamp-2 md:text-lg md:leading-normal md:line-clamp-1">
                                                    {product.name}
                                                </h3>
                                                <p className="font-display text-base font-semibold text-gradient mt-0.5 md:text-xl md:mt-1">
